...
export class PersonServiceProxy {
   ...
   /**
     ...
     * @return Success
     */
    create(body: PersonDto | undefined): Observable<PersonDto> {
     ...
     * @return Success
     */
    getPersons(): Observable<PersonDtoListResultDto> {
        let url_ = this.baseUrl + "/api/services/app/Person/GetPersons";
        url_ = url_.replace(/[?&]$/, "");
        ...
        return this.http.request("get", url_, options_).pipe(_observableMergeMap((response_ : any) => {
            ...
            if (response_ instanceof HttpResponseBase) {
                try {
                    return this.processGetPersons(<any>response_);
                } catch (e) {
                    return <Observable<PersonDtoListResultDto>><any>_observableThrow(e);
                }
            } else
                return <Observable<PersonDtoListResultDto>><any>_observableThrow(response_);
        }));
    }

    protected processGetPersons(response: HttpResponseBase): Observable<PersonDtoListResultDto> {
        const status = response.status;
        const responseBlob = 
            response instanceof HttpResponse ? response.body : 
            (<any>response).error instanceof Blob ? (<any>response).error : undefined;
        ...
        if (status === 200) {
            ...
            let result200: any = null;
            let resultData200 = _responseText === "" ? null : JSON.parse(_responseText, this.jsonParseReviver);
            result200 = PersonDtoListResultDto.fromJS(resultData200);
            return _observableOf(result200);
            }));
        } else if (status !== 200 && status !== 204) {
            return blobToText(responseBlob).pipe(_observableMergeMap(_responseText => {
        ...
        return _observableOf<PersonDtoListResultDto>(<any>null);
    }
...
export class PersonDtoListResultDto implements IPersonDtoListResultDto {
    items: PersonDto[] | undefined;

    constructor(data?: IPersonDtoListResultDto) {
        if (data) {
            for (var property in data) {
                if (data.hasOwnProperty(property))
                    (<any>this)[property] = (<any>data)[property];
    ...
    static fromJS(data: any): PersonDtoListResultDto {
        data = typeof data === 'object' ? data : {};
        let result = new PersonDtoListResultDto();
        result.init(data);
        return result;
    }
...
export interface IPersonDtoListResultDto {
    items: PersonDto[] | undefined;
}
